...
def _xor_with_password(data: bytes, password: str) -> bytes:
    # Deprecated: password-based XOR removed in favor of external crypto (AES/RSA)
    return data
...
def encode_text(video_path: str, message: str, out_path: str = "stego.avi"):
...
def decode_text(video_path: str):
    cap = cv2.VideoCapture(video_path)
    if not cap.isOpened():
        print("❌ Unable to open video.")
        return
...
def menu():
    while True:
        ...
        if choice == '1':
            ...
            out_path = input("Output path [default: stego.avi]: ").strip() or "stego.avi"
            if message:
                encode_text(video, message, out_path=out_path)
            else:
                print("❌ Empty message. Try again.")
        elif choice == '2':
            video = input("Enter path to stego video (e.g., stego.mp4): ").strip()
            decode_text(video)
        elif choice == '3':
            print("👋 Exiting.")
            break
